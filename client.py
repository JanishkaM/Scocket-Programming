...
class BasicChatUI:
    def __init__(self, root):
        ...
        self.users_listbox = tk.Listbox(
            ...
            height=1,
            exportselection=0 # Important for not deselecting when clicking on other widgets
        )
    ...
    def connect_to_server(self):
        try:
            ...
            messagebox.showerror("Connection Error", f"Could not connect to server at {SERVER}:{PORT}\n{e}")
            self.root.quit()
    ...
    def send_message(self):
        ...
        self.message_entry.delete("1.0", tk.END) # Clear input regardless
        if not msg:
            return
        ...
        try:
            ...
            # No longer displaying own message here; the server will echo it back if needed
            # For simplicity, we'll display our own message immediately:
    ...
    def send_message_enter(self, event):
        ...
        if not (event.state & 0x1):  # Check if Shift is not pressed
            self.send_message()
            return "break" # Prevent default text insert for Return
    ...
    def display_own_message(self, message):
        ...
        self.chat_display.insert(tk.END, f"{self.username} (You): ", ("sender", "self_sender"))
    ...
    def display_received_message(self, full_message):
        ...
        sender_tag = "sender"
        message_tag = "message"
        sender_part = "Server" # Default for server messages
        message_part = full_message
        
        # Check for message format [SenderName/Addr] Message
    ...
    def disconnect(self):
        ...
        if self.connected:
            try:
                ...
                pass # Connection already lost, ignore send error
                
            self.connected = False
            self.client.close()
            self.update_status("Disconnected from server", "red")
            self.display_system_message("Disconnected from server. Close window to exit.")
